...
| `Missing child element(s)` | A required child is absent, such as an empty `<body>` |
...
See [webui.md](webui.md) for what the viewer does.

### `[Errno 10048] error while attempting to bind on address ('127.0.0.1', 8000)`

Something is already listening on port 8000, usually an earlier copy of the
viewer that was never stopped. On Linux and macOS the message is
`Address already in use`.

**Action:** if `http://127.0.0.1:8000` already shows the viewer, use that one.
Otherwise stop whichever process holds the port, or change the port in
`webui/run.py`.

### A change does not show up

The server does not reload. A change to `app.py` or `akn_view.py` needs a
restart. A change to a template needs only a refresh. A change to
`static/app.js` or `static/style.css` may need a hard refresh (Ctrl+F5), as
browsers can keep serving the old copy of a static file after a normal one.

### Convert seems to hang

It is still working. The request only returns when conversion is done, which
takes about half a minute for the BBMP Act. The button stays disabled with a
note under it until then. For more than a few documents, use the CLI instead:
it prints each report as it goes.

### Garbled text with amber shading

This is text in a legacy font encoding, not a rendering fault. See
[languages.md](languages.md#text-in-the-other-language). The same text looks
the same in the `.xml` file.

### No "unresolved citations" in the Metadata tab

The count exists only while a PDF is being converted, and nothing is kept of
it afterwards. Convert the document with the CLI to see it.
